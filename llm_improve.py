...
- create_weekly_overview(): school + calendar data are sent to the LLM, which produces the full weekly digest (title, intro, Skola, Kalender). Preferred path.
- With USE_LLM_EXTRACTION=1: school content is extracted from raw page text per page; then calendar is merged and create_weekly_overview builds the digest in one LLM call.
